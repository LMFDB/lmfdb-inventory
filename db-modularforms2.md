...
| Field | type | Example| Description |
 ...
 | `latex_name`: | `\\chi_{1}(1, \\cdot)` | latex_string |
...
## Collection webchar.chunks
    Description: Chunks for the GridFS files described in webchar.files
...
## Collection Atkin_Lehner.files 
 ...
 | `N` | integer |  17 | level |
  ...
  | `chunkSize` | integer | 261120 | size of chunks for this collection |
  | `cputime`  | float | 5.744359 | time to compute the eigenvalues |
  | `filename` | string  | `atkin_lehner_evs-   17-  2-  0-  0` | filename in gridfs |
  | `k` | integer |  2 | weight |
  | `length` | integer | 14 | size of this file in bytes |
  | `md5`:  | `3bc1e9f9479b228be455336a2f4f0e6d` | hash of the file |
  ...
  | `uploadDate` | datetime |  datetime.datetime(2014, 6, 4, 22, 33, 52, 387000)} | date this file was uploaded|
 
## Collection Atkin_Lehner.chunks
    Description: chunks for the GridFS files associated to the collection Atkin_Lehner.files
...
## Collection Modular_symbols.files'
 ...
 |-------|------|--------|---------------|
  ...
  | `k` |  integer | 2 | weight |
  | `length` |  integer | 259255 | length of the file |
  | `md5` |  | '110ace810060c1d1e6e391e9c83e6b03',
  ...
  | `uploadDate` | datetime.datetime(2015, 7, 30, 0, 5, 5, 206000) | date the record was uploaded
  
## Collection Modular_symbols.chunks  
    Description: Chunks for the GridFS files in Modular_symbols.files
  
## Collection Newform_factors.files
    Description: GridFS files containing newform factors.
    File format: Sage object of type 'sage.modular.modsym.subspace.ModularSymbolsSubspace_with_categor'
...
## Collection Modular_symbols.chunks  
    Description: Chunks for the GridFS files in Newform_factors.files
